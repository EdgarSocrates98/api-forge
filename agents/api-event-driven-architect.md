...
description: Arquitetura assíncrona — postura de SQS/SNS/EventBridge/MSK/Step Functions em dumps offline, DLQ e regras AF-MSG, consumer lag e backlog como medidas declaradas. Entra quando a pergunta é fila/tópico/evento/orquestração.
...
1. `af-inventory` — dumps de messaging no disco; ausência medida.
2. `af-extractor` — `model sqs`/`sns`/`eventbridge`/`msk`/`stepfunctions`.
...
Não publica nem consome mensagens — postura lida de dumps.
