...
_OPS = {
    ...
    "aggregate": re.compile(
        r"\b(aggs|aggregations|group\s+by|count\(|sum\(|avg\()\b", re.IGNORECASE
    ),
...
_NAME_RE = re.compile(
    r"(?:index|table|IndexName|TableName)\s*[=:,]\s*[\"']([^\"']+)", re.IGNORECASE
)
...
def _fact(kind: str, rel: str, digest: str, line: int, **measures: Any) -> Fact:
    return Fact(
        ...
        kind=kind,
        source=SourceRef(path=rel, sha256=digest, line=line, extractor="analytical"),
...
def extract_analytical(project_root: Path, engine: str = "opensearch") -> CodeInventory:
    ...
    for path in sorted(root.rglob("*")):
        if (
            not path.is_file()
            or path.is_symlink()
            or path.suffix.lower() not in {".py", ".java", ".go", ".ts", ".js", ".sql", ".json"}
        ):
            continue
        try:
            ...
            diagnostics.append(
                Diagnostic(
                    code="AF-ANALYTICAL-READ",
                    status=FindingStatus.UNRESOLVED,
                    message=f"{path}: {exc}",
                    source=SourceRef(
                        path=str(path.relative_to(root)), sha256="0" * 64, extractor="analytical"
                    ),
                )
            )
            continue
        lowered = text.lower()
        sql_candidate = (
            path.suffix.lower() == ".sql" and engine == "redshift" and "select" in lowered
        )
        ...
        if not any(
            fact.source.path == rel and fact.kind == "data.analytical.name" for fact in facts
        ):
            diagnostics.append(
                Diagnostic(
    ...
    return CodeInventory(
        ...
        execution=static_execution(
            ...
            tuple(diagnostics),
            limitations=(
                "does not execute search or SQL",
                "does not prove shard or partition health",
            ),
        ),
    )


def build_analytical_ir(
    inventory: CodeInventory, *, engine: str, provider: str = "aws"
) -> AnalyticalAccessIR:
    names = tuple(
        sorted(
            {str(f.measures["name"]) for f in inventory.facts if f.kind == "data.analytical.name"}
        )
    )
    operations = tuple(
        sorted(
            {
                ...
                if f.kind == "data.analytical.operation"
            }
        )
    )
    risks: set[str] = set()
    ...
    return AnalyticalAccessIR(
        ...
        engine=cast(Literal["opensearch", "redshift"], engine),
        provider=provider,
        indexes_or_tables=names,
