"""Go extractor: *.go files -> CodeInventory via tree-sitter."""
...
from apiforge.adapters.go.scan import scan_source
...
def _diag(code: str, message: str, rel: str, digest: str, line: int | None) -> Diagnostic:
    return Diagnostic(
        ...
        message=message,
        source=SourceRef(path=rel, sha256=digest, line=line, extractor="go"),
    )


def extract_go(project_root: Path) -> CodeInventory:
    """Statically extract Go routes; unresolvable shapes are named, not guessed."""
        ...
        for route in scan.routes:
            facts.append(
                Fact(
                    ...
                    kind="code.route",
                    source=SourceRef(path=rel, sha256=digest, line=route.line, extractor="go"),
