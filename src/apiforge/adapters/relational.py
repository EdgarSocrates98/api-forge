...
_CALL_RE = re.compile(
    r"\b(execute|executemany|query|raw|fetchone|fetchall|begin|commit|rollback)\s*\(", re.IGNORECASE
)
_POOL_RE = re.compile(
    r"\b(pool|connectionpool|hikaricp|sqlalchemy\.create_engine|pgbouncer)\b", re.IGNORECASE
)
...
def extract_relational(project_root: Path, database: str = "postgres") -> CodeInventory:
    ...
    for path in sorted(root.rglob("*")):
        if (
            not path.is_file()
            or path.is_symlink()
            or path.suffix.lower() not in {".py", ".java", ".go", ".sql", ".ts", ".js"}
        ):
            continue
        try:
        ...
        if _POOL_RE.search(text):
            facts.append(
                _fact("data.relational.pool", rel, digest, 1, database=database, declared=True)
            )
        if _TX_RE.search(text):
            facts.append(
                _fact(
                    "data.relational.transaction", rel, digest, 1, database=database, declared=True
                )
            )
        if path.suffix.lower() != ".sql" and not any(
            fact.source.path == rel
            and fact.kind in {"data.relational.query", "data.relational.operation"}
            for fact in facts
        ):
            diagnostics.append(
                Diagnostic(
