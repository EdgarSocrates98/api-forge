...
_OPS = {
    "producer": re.compile(r"\b(KafkaProducer|Producer|produce|Produce|send|publish|WriteMessages)\b", re.IGNORECASE),
    "consumer": re.compile(r"\b(KafkaConsumer|Consumer|consume|Consume|subscribe|ReadMessage|ReadMessages|poll)\b", re.IGNORECASE),
    "commit": re.compile(r"\b(commit|Commit|commitSync|commitAsync)\b"),
    "transaction": re.compile(r"\b(beginTransaction|commitTransaction|abortTransaction|transactional\.id)\b"),
...
def extract_streaming(project_root: Path, broker: str = "kafka") -> CodeInventory:
    ...
    for path in sorted(root.rglob("*")):
        if not path.is_file() or path.is_symlink() or path.suffix.lower() not in {".py", ".java", ".go", ".ts", ".js", ".yaml", ".yml", ".properties"}:
            continue
        try:
            ...
            diagnostics.append(Diagnostic(
                ...
                message=f"{path}: {exc}",
                source=SourceRef(path=str(path.relative_to(root)), sha256="0" * 64, extractor="streaming"),
            ))
            continue
        lowered = text.lower()
        ...
        for match in _TOPIC_RE.finditer(text):
            line = text.count("\n", 0, match.start()) + 1
            facts.append(_fact("data.streaming.topic", rel, digest, line, broker=broker, topic=match.group(1)))
        for match in _GROUP_RE.finditer(text):
            line = text.count("\n", 0, match.start()) + 1
            facts.append(_fact("data.streaming.consumer_group", rel, digest, line, broker=broker, group=match.group(1)))
...
def build_streaming_ir(inventory: CodeInventory, *, broker: str = "kafka", provider: str = "kafka|msk") -> StreamingAccessIR:
    ...
    return StreamingAccessIR(
        ...
        broker=broker_name, provider=provider, topics=topics, consumer_groups=groups,
        roles=tuple(sorted(roles)), operations=operations, delivery_signals=delivery,
