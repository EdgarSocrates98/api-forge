...
def brief_for_grpc_verification(
    intention: str, verdict: str, evidence: tuple[str, ...], gaps: tuple[str, ...] = ()
) -> OutcomeBrief:
    """Render gRPC verification into the same DONE/REVIEW/BLOCKED vocabulary."""
    if verdict == "DONE" and not gaps and evidence:
        return OutcomeBrief(
            status=BriefStatus.DONE, outcome=intention, proof=evidence, subject="grpc"
        )
    ...
    return OutcomeBrief(
        ...
        outcome=intention,
        proof=evidence,
        gaps=gaps or ("gRPC verification requires review",),
        subject="grpc",
    )
