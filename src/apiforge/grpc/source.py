...
def load_source(path: Path) -> GrpcIR:
    ...
    if path.suffix in {".json", ".fds"}:
        ...
        return GrpcIR(
            ...
            provenance=(f"descriptor:{path}:{digest}",),
            unresolved=(str(loaded.get("reason", "descriptor-unresolved")),),
        )
...
def _ir_from_descriptor(path: Path, digest: str, descriptor: dict[str, object]) -> GrpcIR:
    ...
    messages = tuple(
        GrpcMessage(
            name=str(item.get("name", "")), full_name=f"{package}.{item.get('name', '')}".strip(".")
        )
    ...
    for item in raw_services:
        ...
        for rpc_item in raw_rpcs:
            ...
            rpcs.append(
                GrpcRpc(
                    name=rpc_name,
                    full_name=f"{full_service}/{rpc_name}",
                    request_type=str(rpc_item.get("request", "")),
                    response_type=str(rpc_item.get("response", "")),
                    stream_mode=mode,
                    http_method=str(rpc_item["http_method"])
                    if rpc_item.get("http_method")
                    else None,
                    http_path=str(rpc_item["http_path"]) if rpc_item.get("http_path") else None,
                    source_path=str(path),
                )
            )
