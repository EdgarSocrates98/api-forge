...
class DatadogAdapter:
    ...
    def capabilities(self) -> tuple[Capability, ...]:
        return (Capability(name="monitor", supported=True), Capability(name="slo", supported=True), Capability(name="dashboard", supported=True), Capability(name="apply", supported=False, read_only=False, reason="broker adapter not configured"))
    ...
    def apply(self, intent: VendorIntent, dry_run: bool = True) -> OperationReceipt:
        ...
        return OperationReceipt(operation_id=f"op:{intent.id}", intent_id=intent.id, status=status, provider=self.name, reason=reason)
