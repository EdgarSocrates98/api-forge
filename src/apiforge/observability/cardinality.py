...
def findings(
    records: tuple[TelemetryRecord, ...], budget: int = 100
) -> tuple[ObservabilityFinding, ...]:
    result = []
        ...
        if value > budget:
            result.append(
                ObservabilityFinding(
                    id=f"cardinality:{record.id}",
                    severity="high",
                    title="Cardinality budget exceeded",
                    detail=f"{value}>{budget}",
                    evidence=(record.id,),
                    status="confirmed",
                )
            )
