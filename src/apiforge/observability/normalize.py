...
def normalize_records(records: Iterable[Mapping[str, object]], source: str = "fixture") -> tuple[TelemetryRecord, ...]:
    ...
    for index, item in enumerate(records):
        ...
        normalized.append(
            TelemetryRecord(
                ...
                duration_ms=float(str(item["duration_ms"])) if item.get("duration_ms") is not None else None,
                status_code=int(str(item["status_code"])) if item.get("status_code") is not None else None,
