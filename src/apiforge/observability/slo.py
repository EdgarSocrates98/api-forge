...
def evaluate_slo(definition: SLODefinition, records: Iterable[TelemetryRecord]) -> SLOResult:
    ...
    if definition.indicator == "latency":
        good = [record for record in selected if record.duration_ms is not None and definition.threshold_ms is not None and record.duration_ms <= definition.threshold_ms]
    else:
        good = [record for record in selected if record.status_code is not None and record.status_code < 500]
    total = len(selected)
    if not total or not good and definition.indicator == "latency" and definition.threshold_ms is None:
        return SLOResult(slo_id=definition.id, status="inconclusive", good_events=len(good), total_events=total, objective=definition.objective, limitations=("insufficient or missing telemetry",))
