...
def test_distribution_contracts_are_versioned_and_closed() -> None:
    paths = ForgePaths(package_root="/pkg", executable="/bin/python", state_root="/state", cache_root="/cache")
...
def test_distribution_contract_rejects_unknown_fields() -> None:
    try:
        ForgePaths(package_root="/pkg", executable="/bin/python", state_root="/state", cache_root="/cache", extra=True)
