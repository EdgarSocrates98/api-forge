...
def test_plan_has_dependency_dag_and_conservative_status() -> None:
    ...
    spec = MigrationSpec(project_root=str(root), ecosystem="go", source_version="1.21", target_version="1.24")
...
def test_blocking_finding_cannot_be_done() -> None:
    ...
    spec = MigrationSpec(project_root=str(root), ecosystem="python", source_version="2", target_version="3")
    discovery = discover(spec)
    plan = compile_plan(spec, discovery)
    report = verify_report(MigrationReport(spec_identity=spec.identity(), discovery=discovery, plan=plan), evidence_ok=True, verification_ok=True)
    assert report.outcome is not None
